            ...
            if (this.IsSessionOpenNotificationEnabled)
            {
                 ...
                 || methodInfo.GetParameters().Length > 0)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        Res.GetString(Res.S("ContractIsNotSelfConsistentWhenIsSessionOpenNotificationEnabled"), operationName, "Action", OperationDescription.SessionOpenedAction, "IsOneWay", "IsInitiating")));
                }
            }
        }
    }
