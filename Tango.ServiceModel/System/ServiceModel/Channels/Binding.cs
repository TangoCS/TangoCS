...
namespace System.ServiceModel.Channels
{
    ...
    public abstract class Binding : IDefaultCommunicationTimeouts
    {
        ...
        protected Binding(string name, string ns)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("name", Res.GetString(Res.S("SFXBindingNameCannotBeNullOrEmpty")));
            }
            if (ns == null)
            {
        ...
        public string Name
        {
            ...
            set
            {
                if (string.IsNullOrEmpty(value))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("value", Res.GetString(Res.S("SFXBindingNameCannotBeNullOrEmpty")));
        ...
        void ValidateSecurityCapabilities(ISecurityCapabilities runtimeSecurityCapabilities, BindingParameterCollection parameters)
        {
            ...
            if (!SecurityCapabilities.IsEqual(bindingSecurityCapabilities, runtimeSecurityCapabilities))
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new InvalidOperationException(Res.GetString(Res.S("SecurityCapabilitiesMismatched"), this)));
            }
        }
            ...
            if (transport == null)
            {
                if (contractName == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        Res.GetString(Res.S("CustomBindingRequiresTransport"), this.Name)));
                }
                else
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        Res.GetString(Res.S("SFxCustomBindingNeedsTransport1"), contractName)));
                }
            }
            if (index != elements.Count - 1)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                    Res.GetString(Res.S("TransportBindingElementMustBeLast"), this.Name, transport.GetType().Name)));
            }
            if (string.IsNullOrEmpty(transport.Scheme))
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                    Res.GetString(Res.S("InvalidBindingScheme"), transport.GetType().Name)));
            }

            if (this.MessageVersion == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                    Res.GetString(Res.S("MessageVersionMissingFromBinding"), this.Name)));
            }
        }
