...
namespace System.ServiceModel.Channels
{
	...
	public sealed class MessageHeaders : IEnumerable<MessageHeaderInfo>
    {
            ...
            get
            {
                if (index < 0 || index >= headerCount)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        ...
                        Res.GetString(Res.S("ValueMustBeInRange"), 0, headerCount)));
                }
                ...
                if ((object)headers[i].HeaderInfo == (object)headerInfo)
                {
                    if ((headers[i].HeaderProcessing & HeaderProcessing.Understood) != 0)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(
                            Res.GetString(Res.S("HeaderAlreadyUnderstood"), headerInfo.Name, headerInfo.Namespace), "headerInfo"));
                    }
        ...
        public void CopyHeaderFrom(MessageHeaders collection, int headerIndex)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("MessageHeaderVersionMismatch"), collection.version.ToString(), version.ToString()), "collection"));
            }
            ...
            if (headerIndex < 0 || headerIndex >= collection.headerCount)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    ...
                    Res.GetString(Res.S("ValueMustBeInRange"), 0, collection.headerCount)));
            }
            ...
            switch (header.HeaderType)
            {
                ...
                case HeaderType.WriteableHeader:
                    ...
                    break;
                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("InvalidEnumValue"), header.HeaderType)));
            }
        }
        ...
        public void CopyTo(MessageHeaderInfo[] array, int index)
        {
            ...
            if (index < 0 || (index + headerCount) > array.Length)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new ArgumentOutOfRangeException("index", index,
                    Res.GetString(Res.S("ValueMustBeInRange"), 0, array.Length - headerCount)));
            }
            for (int i = 0; i < headerCount; i++)
        ...
        Exception CreateDuplicateHeaderException(HeaderKind kind)
        {
            ...
            switch (kind)
            {
                ...
                case HeaderKind.To:
                    ...
                    break;
                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("InvalidEnumValue"), kind)));
            }

            return new MessageHeaderException(
                Res.GetString(Res.S("MultipleMessageHeaders"), name, this.version.Addressing.Namespace),
                name,
            ...
            for (int i = 0; i < headerCount; i++)
            {
                if (headers[i].HeaderKind != HeaderKind.Unknown)
                {
                    ...
                    if (info.Name == name && info.Namespace == ns)
                    {
                        ...
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                new MessageHeaderException(Res.GetString(Res.S("MultipleMessageHeaders"), name, ns), name, ns, true));
                        }
                        foundAt = i;
                    }
                }
                    ...
                    if (info.Name == name && info.Namespace == ns)
                    {
                        for (int j = 0; j < actors.Length; j++)
                        {
                            if (actors[j] == info.Actor)
                            {
                                ...
                                {
                                    if (actors.Length == 1)
                                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MessageHeaderException(Res.GetString(Res.S("MultipleMessageHeadersWithActor"), name, ns, actors[0]), name, ns, true));
                                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MessageHeaderException(Res.GetString(Res.S("MultipleMessageHeaders"), name, ns), name, ns, true));
                                }
                                foundAt = i;
                            }
                        }
                        ...
                        if (foundValue != null)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                new MessageHeaderException(
                                    Res.GetString(Res.S("MultipleRelatesToHeaders"), relationshipType.AbsoluteUri),
        ...
        public T GetHeader<T>(string name, string ns, params string[] actors)
        {
            ...
            if (index < 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MessageHeaderException(Res.GetString(Res.S("HeaderNotFound"), name, ns), name, ns));
        ...
        void ValidateHeaderKind(HeaderKind headerKind)
        {
            if (this.version.Envelope == EnvelopeVersion.None)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(Res.GetString(Res.S("HeadersCannotBeAddedToEnvelopeVersion"), this.version.Envelope)));
                }
            }
            ...
            if (this.version.Addressing == AddressingVersion.None)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(Res.GetString(Res.S("AddressingHeadersCannotBeAddedToAddressingVersion"), this.version.Addressing)));
                }
            }
        }
        ...
        public XmlDictionaryReader GetReaderAtHeader(int headerIndex)
        {
            ...
            switch (headers[headerIndex].HeaderType)
            {
                ...
                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("InvalidEnumValue"), headers[headerIndex].HeaderType)));
            }
        }
        ...
        internal MessageHeader GetMessageHeader(int index)
        {
            ...
            switch (headers[index].HeaderType)
            {
                ...
                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("InvalidEnumValue"), headers[index].HeaderType)));
            }
        }
        ...
        public void Insert(int headerIndex, MessageHeader header)
        {
            ...
            if (!header.IsMessageVersionSupported(this.version))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("MessageHeaderVersionNotSupported"),
        ...
        internal void RemoveUnderstood(MessageHeaderInfo headerInfo)
        {
            ...
            for (int i = 0; i < headerCount; i++)
            {
                if ((object)headers[i].HeaderInfo == (object)headerInfo)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(
                            Res.GetString(Res.S("HeaderAlreadyNotUnderstood"), headerInfo.Name, headerInfo.Namespace), "headerInfo"));
                    }
        ...
        static void TraceUnderstood(MessageHeaderInfo info)
        {
            ...
            //        Res.GetString(Res.S("TraceCodeUnderstoodMessageHeader")),
