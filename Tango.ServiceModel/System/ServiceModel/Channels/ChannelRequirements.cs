   ...
   //                 channelType == typeof(IRequestSessionChannel));
        ...
        public static Exception CantCreateListenerException(IEnumerable<Type> supportedChannels, IEnumerable<Type> requiredChannels, string bindingName)
        {
            ...
            if (exception == null)
            {
                ...
                exception = new InvalidOperationException(Res.GetString(Res.S("EndpointListenerRequirementsCannotBeMetBy3"),
        ...
        public static Exception CantCreateChannelException(IEnumerable<Type> supportedChannels, IEnumerable<Type> requiredChannels, string bindingName)
        {
            ...
            {
                // none of the obvious speculations about the failure holds, so we fall back to the generic error message
                exception = new InvalidOperationException(Res.GetString(Res.S("CouldnTCreateChannelForType2"), bindingName, contractChannelTypesString));
            }
            ...
            string bindingName, ref string contractChannelTypesString, ref string bindingChannelTypesString)
        {
            ...
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesnTSupportAnyChannelTypes1"), bindingName));
            }
            if (contractRequiresSession && !bindingSupportsSession)
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesnTSupportSessionButContractRequires1"), bindingName));
            }
            if (contractRequiresDatagram && !bindingSupportsDatagram)
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesntSupportDatagramButContractRequires"), bindingName));
            }
            if (contractRequiresDuplex && !bindingSupportsDuplex)
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesnTSupportDuplexButContractRequires1"), bindingName));
            }
            if (contractRequiresRequestReply && !bindingSupportsRequestReply)
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesnTSupportRequestReplyButContract1"), bindingName));
            }
            if (contractRequiresOneWay && !bindingSupportsOneWay)
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesnTSupportOneWayButContractRequires1"), bindingName));
            }
            if (contractRequiresTwoWay && !bindingSupportsTwoWay)
            {
                return new InvalidOperationException(Res.GetString(Res.S("BindingDoesnTSupportTwoWayButContractRequires1"), bindingName));
            }
