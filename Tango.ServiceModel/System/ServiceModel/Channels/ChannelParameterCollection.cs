    ...
    public class ChannelParameterCollection : Collection<object>
    {
            ...
            if (channel != null)
            {
                ...
                switch (state)
                {
                    ...
                    case CommunicationState.Faulted:
                        text = Res.GetString(Res.S("ChannelParametersCannotBeModified"),
                                            channel.GetType().ToString(), state.ToString());
                        break;

                    default:
                        text = Res.GetString(Res.S("CommunicationObjectInInvalidState"),
                                            channel.GetType().ToString(), state.ToString());
                        break;
                }
        ...
        void ThrowIfMutable()
        {
            ...
            if (channel != null)
            {
                ...
                {
                    case CommunicationState.Created:
                        text = Res.GetString(Res.S("ChannelParametersCannotBePropagated"),
                                            channel.GetType().ToString(), state.ToString());
                        break;
