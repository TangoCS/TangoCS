...
namespace System.ServiceModel.Channels
{
    ...
    public class HttpMessageHandlerFactory
    {
        ...
        public HttpMessageHandlerFactory(params Type[] handlers)
        {
            ...
            {
                throw FxTrace.Exception.Argument("handlers", Res.GetString(Res.S("InputTypeListEmptyError")));
            }
            ...
            for (int cnt = 0; cnt < handlers.Length; cnt++)
            {
                ...
                if (handler == null)
                {
                    throw FxTrace.Exception.Argument(
                        ...
                        Res.GetString(Res.S("HttpMessageHandlerTypeNotSupported"), "null", delegatingHandlerType.Name));
                }
                ...
                if (!delegatingHandlerType.IsAssignableFrom(handler) || handler.IsAbstract)
                {
                    throw FxTrace.Exception.Argument(
                        string.Format(CultureInfo.InvariantCulture, "handlers[<<{0}>>]", cnt),
                        Res.GetString(Res.S("HttpMessageHandlerTypeNotSupported"), handler.Name, delegatingHandlerType.Name));
                }
        ...
        //        {
        //            throw FxTrace.Exception.AsError(new ConfigurationErrorsException(Res.GetString(Res.S("HttpMessageHandlerFactoryConfigInvalid_WithBothTypeAndHandlerList"), ConfigurationStrings.MessageHandlerFactory, ConfigurationStrings.Type, ConfigurationStrings.Handlers)));
        //        }

        //        Type factoryType = HttpChannelUtilities.GetTypeFromAssembliesInCurrentDomain(configElement.Type);
        //        if (factoryType == null)
        //        {
        //            throw FxTrace.Exception.AsError(new ConfigurationErrorsException(Res.GetString(Res.S("CanNotLoadTypeGotFromConfig"), configElement.Type)));
        //        }
        ...
        //                Res.GetString(
        //                Res.S("WebSocketElementConfigInvalidHttpMessageHandlerFactoryType"),
        ...
        //            if (handlerType == null)
        //            {
        //                throw FxTrace.Exception.AsError(new ConfigurationErrorsException(Res.GetString(Res.S("CanNotLoadTypeGotFromConfig"), configElement.Handlers[i].Type)));
        //            }
        ...
        //    {
        //        throw FxTrace.Exception.AsError(new InvalidOperationException(Res.GetString(Res.S("HttpMessageHandlerFactoryWithFuncCannotGenerateConfig"), typeof(HttpMessageHandlerFactory).Name, typeof(Func<IEnumerable<DelegatingHandler>>).Name)));
        //    }
        ...
        protected virtual HttpMessageHandler OnCreate(HttpMessageHandler innerChannel)
        {
                ...
                if (this.handlerFunc != null)
                {
                    ...
                    if (handlerInstances != null)
                    {
                        foreach (DelegatingHandler handler in handlerInstances)
                        {
                            if (handler == null)
                            {
                                throw FxTrace.Exception.Argument("handlers", Res.GetString(Res.S("DelegatingHandlerArrayFromFuncContainsNullItem"), delegatingHandlerType.Name, GetFuncDetails(this.handlerFunc)));
                            }
                        }
                    }
                }
            ...
            if (handlerInstances != null)
            {
                foreach (DelegatingHandler handler in handlerInstances)
                {
                    ...
                    {
                        throw FxTrace.Exception.Argument("handlers", Res.GetString(Res.S("DelegatingHandlerArrayHasNonNullInnerHandler"), delegatingHandlerType.Name, "InnerHandler", handler.GetType().Name));
                    }
