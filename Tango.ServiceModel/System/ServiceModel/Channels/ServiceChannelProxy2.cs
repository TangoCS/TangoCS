...
namespace System.ServiceModel.Channels
{
	public class ServiceChannelProxy : DispatchProxy, ICommunicationObject, IChannel, IClientChannel, IOutputChannel, IRequestChannel, IServiceChannel//, IDuplexContextChannel
	{
		...
		internal static TChannel CreateProxy<TChannel>(MessageDirection direction, ServiceChannel serviceChannel)
		{
			...
			if (proxy == null)
			{
				throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException("FailedToCreateTypedProxy"));
			}
			...
			else
			{
				...
				if (operation == null)
				{
					if (_serviceChannel.Factory != null)
						throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException("SFxMethodNotSupported1"));
					else
						throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException("SFxMethodNotSupportedOnCallback1"));
				}
		...
		protected override object Invoke(MethodInfo targetMethod, object[] args)
		{
			...
			{
				throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException("InvalidTypedProxyMethodHandle"));
			}
