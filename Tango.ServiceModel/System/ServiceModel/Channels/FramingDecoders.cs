...
namespace System.ServiceModel.Channels
{
    ...
    static class DecoderHelper
    {
        public static void ValidateSize(int size)
        {
            if (size <= 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("size", size, Res.GetString(
                    Res.S("ValueMustBePositive"))));
            }
        }
    }
    ...
    struct IntDecoder
    {
        ...
        public int Value
        {
            ...
            {
                if (!isValueDecoded)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("FramingValueNotAvailable"))));
                return value;
            }
        }
        ...
        public int Decode(byte[] buffer, int offset, int size)
        {
            ...
            while (bytesConsumed < size)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataException(Res.GetString(Res.S("FramingSizeTooLarge"))));
                }
                index++;
                if ((next & 0x80) == 0)
                {
            ...
            switch (currentState)
            {
                case State.ReadingSize:
                    ...
                    if (sizeDecoder.IsValueDecoded)
                    {
                        ...
                        {
                            encodedBytes = new byte[encodedSize];
                            value = null;
                        }
                ...
                case State.ReadingBytes:
                    ...
                    break;
                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataException(Res.GetString(Res.S("InvalidDecoderStateMachine"))));
            }
    ...
    class ViaStringDecoder : StringDecoder
    {
        ...
        {
            Exception result = new InvalidDataException(Res.GetString(Res.S("FramingViaTooLong"), size));
        ...
        protected override void OnComplete(string value)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataException(Res.GetString(Res.S("FramingViaNotUri"), value), exception));
            }
        }
    ...
    class FaultStringDecoder : StringDecoder
    {
        ...
        protected override Exception OnSizeQuotaExceeded(int size)
        {
            return new InvalidDataException(Res.GetString(Res.S("FramingFaultTooLong"), size));
        }
        ...
        public static Exception GetFaultException(string faultString, string via, string contentType)
        {
            ...
            {
                return new EndpointNotFoundException(Res.GetString(Res.S("EndpointNotFound"), via));
            }
            else if (faultString == FramingEncodingString.ContentTypeInvalidFault)
            {
                return new ProtocolException(Res.GetString(Res.S("FramingContentTypeMismatch"), contentType, via));
            }
            else if (faultString == FramingEncodingString.ServiceActivationFailedFault)
            {
                return new ServiceActivationException(Res.GetString(Res.S("Hosting_ServiceActivationFailed"), via));
			}
            else if (faultString == FramingEncodingString.ConnectionDispatchFailedFault)
            {
                return new CommunicationException(Res.GetString(Res.S("Sharing_ConnectionDispatchFailed"), via));
			}
            else if (faultString == FramingEncodingString.EndpointUnavailableFault)
            {
                return new EndpointNotFoundException(Res.GetString(Res.S("Sharing_EndpointUnavailable"), via));
			}
            else if (faultString == FramingEncodingString.MaxMessageSizeExceededFault)
            {
                Exception inner = new QuotaExceededException(Res.GetString(Res.S("FramingMaxMessageSizeExceeded")));
                return new CommunicationException(inner.Message, inner);
            }
            else if (faultString == FramingEncodingString.UnsupportedModeFault)
            {
                return new ProtocolException(Res.GetString(Res.S("FramingModeNotSupportedFault"), via));
            }
            else if (faultString == FramingEncodingString.UnsupportedVersionFault)
            {
                return new ProtocolException(Res.GetString(Res.S("FramingVersionNotSupportedFault"), via));
            }
            else if (faultString == FramingEncodingString.ContentTypeTooLongFault)
            {
                Exception inner = new QuotaExceededException(Res.GetString(Res.S("FramingContentTypeTooLongFault"), contentType));
                return new CommunicationException(inner.Message, inner);
            }
            else if (faultString == FramingEncodingString.ViaTooLongFault)
            {
                Exception inner = new QuotaExceededException(Res.GetString(Res.S("FramingViaTooLongFault"), via));
                return new CommunicationException(inner.Message, inner);
            }
            else if (faultString == FramingEncodingString.ServerTooBusyFault)
            {
                return new ServerTooBusyException(Res.GetString(Res.S("ServerTooBusy"), via));
            }
            else if (faultString == FramingEncodingString.UpgradeInvalidFault)
            {
                return new ProtocolException(Res.GetString(Res.S("FramingUpgradeInvalid"), via));
            }
            else
            {
                return new ProtocolException(Res.GetString(Res.S("FramingFaultUnrecognized"), faultString));
            }
        }
    }
    ...
    class ContentTypeStringDecoder : StringDecoder
    {
        ...
        protected override Exception OnSizeQuotaExceeded(int size)
        {
            Exception result = new InvalidDataException(Res.GetString(Res.S("FramingContentTypeTooLong"), size));
    ...
    abstract class FramingDecoder
    {
        ...
        protected void ValidateFramingMode(FramingMode mode)
        {
            switch (mode)
            {
                ...
                default:
                    {
                        Exception exception = CreateException(new InvalidDataException(Res.GetString(
                            Res.S("FramingModeNotSupported"), mode.ToString())), FramingEncodingString.UnsupportedModeFault);
        ...
        protected void ValidatePreambleAck(FramingRecordType foundType)
        {
            if (foundType != FramingRecordType.PreambleAck)
            {
                ...
                {
                    exceptionString = Res.GetString(Res.S("PreambleAckIncorrectMaybeHttp"));
                }
                else
                {
                    exceptionString = Res.GetString(Res.S("PreambleAckIncorrect"));
                }
        ...
        {
            return new InvalidDataException(Res.GetString(Res.S("FramingRecordTypeMismatch"), expectedType.ToString(), foundType.ToString()));
        }
        ...
        protected void ValidateMajorVersion(int majorVersion)
        {
            ...
            {
                Exception exception = CreateException(new InvalidDataException(Res.GetString(
                    Res.S("FramingVersionNotSupported"), majorVersion)), FramingEncodingString.UnsupportedVersionFault);
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(exception);
            }
        }
        ...
        {
            return CreateException(new InvalidDataException(Res.GetString(Res.S("FramingPrematureEOF"))));
        }
        ...
        {
            return new ProtocolException(Res.GetString(Res.S("FramingError"), StreamPosition, CurrentStateAsString),
                innerException);
        }
    }
                ...
                switch (currentState)
                {
                    ...
                    default:
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            CreateException(new InvalidDataException(Res.GetString(Res.S("InvalidDecoderStateMachine")))));
                }
        ...
        public int Decode(byte[] bytes, int offset, int size)
        {
            ...
            try
            {
                ...
                switch (currentState)
                {
                    ...
                    case State.End:
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            CreateException(new InvalidDataException(Res.GetString(Res.S("FramingAtEnd")))));
                    default:
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            CreateException(new InvalidDataException(Res.GetString(Res.S("InvalidDecoderStateMachine")))));
                }
