...
namespace System.ServiceModel.Channels
{
	...
	abstract class HttpInput
    {
        ...
        void ThrowMaxReceivedMessageSizeExceeded()
        {
            if (isRequest)
            {
                ThrowHttpProtocolException(Res.GetString(Res.S("MaxReceivedMessageSizeExceeded"), settings.MaxReceivedMessageSize), HttpStatusCode.RequestEntityTooLarge);
            }
            else
            {
                string message = Res.GetString(Res.S("MaxReceivedMessageSizeExceeded"), settings.MaxReceivedMessageSize);
            ...
            try
            {
                ...
                catch (XmlException xmlException)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new ProtocolException(Res.GetString(Res.S("MessageXmlProtocolError")), xmlException));
                }
            }
            finally
            {
        ...
        Message ReadBufferedMessage(Stream inputStream)
        {
            ...
            while (count > 0)
            {
                ...
                if (bytesRead == 0) // EOF 
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new ProtocolException(Res.GetString(Res.S("HttpContentLengthIncorrect"))));
                    }
        ...
        Exception ProcessHttpAddressing(Message message)
        {
            ...
            if (message.Version.Addressing == AddressingVersion.None)
            {
                ...
                if (!actionAbsent)
                {
                    result = new ProtocolException(Res.GetString(Res.S("HttpAddressingNoneHeaderOnWire"),
            ...
            if (isRequest)
            {
                ...
                if (action != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            result = new ActionMismatchAddressingException(Res.GetString(Res.S("HttpSoapActionMismatchFault"),
        ...
        void ValidateContentType()
        {
            ...
            if (string.IsNullOrEmpty(ContentType))
            {
                ThrowHttpProtocolException(Res.GetString(Res.S("HttpContentTypeHeaderRequired")), HttpStatusCode.UnsupportedMediaType, HttpChannelUtilities.StatusDescriptionStrings.HttpContentTypeMissing);
            }
            if (!messageEncoder.IsContentTypeSupported(ContentType))
            {
                ...
                ThrowHttpProtocolException(Res.GetString(Res.S("ContentTypeMismatch"), ContentType, messageEncoder.ContentType), HttpStatusCode.UnsupportedMediaType, statusDescription);
            }
        }
   ...
   //     class ParseMessageAsyncResult : AsyncResult
   //     {
    ...
    abstract class HttpOutput
    {
        ...
        public void ConfigureHttpResponseMessage(Message message, HttpResponseMessage httpResponseMessage, HttpResponseMessageProperty responseProperty)
        {
            ...
            if (message.Version == MessageVersion.None && httpResponseMessagePropertyFound && !string.IsNullOrEmpty(responseProperty.Headers[HttpResponseHeader.ContentType]))
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new ProtocolException(Res.GetString(Res.S("ResponseContentTypeNotSupported"),
                        contentType)));
                }
            }
            ...
            if (httpResponseMessage.Content != null && !string.IsNullOrEmpty(contentType))
            {
                ...
                {
                    throw FxTrace.Exception.Argument("contentType", Res.GetString(Res.S("InvalidContentTypeError"), contentType));
                }
        ...
        public void Send(HttpResponseMessage httpResponseMessage, TimeSpan timeout)
        {
            ...
            //        {
            //            throw FxTrace.Exception.AsError(new TimeoutException(Res.GetString(Res.S("TimeoutOnSend"), timeout)));
            //        }
            //    }
            //}
            else
        ...
        class WebRequestHttpOutput : HttpOutput
        {
            ...
            protected override Stream GetOutputStream()
            {
                try
                {
					...
					IAsyncResult asyncResult = httpWebRequest.BeginGetRequestStream(null, null);
					if (!asyncResult.IsCompleted && !asyncResult.AsyncWaitHandle.WaitOne(100000, false))
					{
						httpWebRequest.Abort();
						throw new WebException("The request timed out", WebExceptionStatus.Timeout);
					}
					var outputStream = httpWebRequest.EndGetRequestStream(asyncResult);
            ...
            protected override bool PrepareHttpSend(Message message)
            {
                    ...
                    for (int i = 0; i < requestHeaders.Count; i++)
                    {
                        ...
                        else if (string.Compare(name, "SOAPAction", StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                        new ProtocolException(Res.GetString(Res.S("HttpSoapActionMismatch"), action, value)));
                                }
                            }
                        }
                        ...
                        else if (string.Compare(name, "if-modified-since", StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                    new ProtocolException(Res.GetString(Res.S("HttpIfModifiedSinceParseError"), value)));
                            }
                        }
                ...
                if (action != null)
                {
                                    ...
                                    || httpWebRequest.ContentType.ToUpperInvariant().IndexOf("ACTION", StringComparison.OrdinalIgnoreCase) != -1)
                                {
                                    try
                                    {
                                        ...
                                        if (parsedContentType.Parameters.ContainsKey("action"))
                                        {
                                            ...
                                            {
                                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                                    new ProtocolException(Res.GetString(Res.S("HttpSoapActionMismatchContentType"), action, value)));
                                            }
                                    ...
                                    {
                                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                            new ProtocolException(Res.GetString(Res.S("HttpContentTypeFormatException"), formatException.Message, httpWebRequest.ContentType), formatException));
                                    }
                                }
                            }
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new ProtocolException(Res.GetString(Res.S("EnvelopeVersionUnknown"),
            ...
            class WebRequestOutputStream : BytesReadPositionStream
            {
                ...
                public override long Position
                {
                    ...
                    set
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException(Res.GetString(Res.S("SeekNotSupported"))));
                    }
                }
        ...
        //            {
        //                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
        //                    new CommunicationObjectAbortedException(Res.GetString(Res.S("HttpResponseAborted")),
    ...
    static class HttpChannelUtilities
    {
        ...
        public static Exception CreateCommunicationException(HttpListenerException listenerException)
        {
            switch (listenerException.NativeErrorCode)
            {
                ...
                //    return new CommunicationException(Res.GetString(Res.S("HttpNoTrackingService"), listenerException.Message), listenerException);

                //case UnsafeNativeMethods.ERROR_NETNAME_DELETED:
                //    return new CommunicationException(Res.GetString(Res.S("HttpNetnameDeleted"), listenerException.Message), listenerException);

                //case UnsafeNativeMethods.ERROR_INVALID_HANDLE:
                //    return new CommunicationObjectAbortedException(Res.GetString(Res.S("HttpResponseAborted")), listenerException);
                ...
                //    return new InsufficientMemoryException(Res.GetString(Res.S("InsufficentMemory")), listenerException);
        ...
        internal static void HandleContinueWithTask(Task task, Action<Exception> exceptionHandler)
        {
            ...
            {
                throw FxTrace.Exception.AsError(new TimeoutException(Res.GetString(Res.S("TaskCancelledError"))));
            }
        }
        ...
        public static void SetRequestTimeout(HttpWebRequest request, TimeSpan timeout)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new TimeoutException(Res.GetString(
                    Res.S("HttpRequestTimedOut"), request.RequestUri, timeout)));
            }
            ...
            out TokenImpersonationLevel impersonationLevel, out AuthenticationLevel authenticationLevel)
        {
            ...
            switch (authenticationScheme)
            {
                ...
                //    {
                //        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                //            new InvalidOperationException(Res.GetString(Res.S("CredentialDisallowsNtlm"))));
                //    }
                //    break;
        ...
        public static HttpWebResponse ProcessGetResponseWebException(WebException webException, HttpWebRequest request, HttpAbortReason abortReason)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new EndpointNotFoundException(Res.GetString(Res.S("EndpointNotFound"), request.RequestUri.AbsoluteUri), webException));
            }

            if (response.StatusCode == HttpStatusCode.ServiceUnavailable)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ServerTooBusyException(Res.GetString(Res.S("HttpServerTooBusy"), request.RequestUri.AbsoluteUri), webException));
            }
            ...
            if (response.StatusCode == HttpStatusCode.UnsupportedMediaType)
            {
                ...
                if (!string.IsNullOrEmpty(statusDescription))
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ProtocolException(Res.GetString(Res.S("MissingContentType"), request.RequestUri), webException));
                    }
                }
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ProtocolException(Res.GetString(Res.S("FramingContentTypeMismatch"), request.ContentType, request.RequestUri), webException));
            }
        ...
        public static Exception ConvertWebException(WebException webException, HttpWebRequest request, HttpAbortReason abortReason)
        {
            switch (webException.Status)
            {
                ...
                case WebExceptionStatus.ProxyNameResolutionFailure:
                    return new EndpointNotFoundException(Res.GetString(Res.S("EndpointNotFound"), request.RequestUri.AbsoluteUri), webException);
                //case WebExceptionStatus.SecureChannelFailure:
                //    return new SecurityNegotiationException(Res.GetString(Res.S("SecureChannelFailure"), request.RequestUri.Authority), webException);
                //case WebExceptionStatus.TrustFailure:
                //    return new SecurityNegotiationException(Res.GetString(Res.S("TrustFailure"), request.RequestUri.Authority), webException);
                case WebExceptionStatus.Timeout:
                    return new TimeoutException(CreateRequestTimedOutMessage(request), webException);
                case WebExceptionStatus.ReceiveFailure:
                    return new CommunicationException(Res.GetString(Res.S("HttpReceiveFailure"), request.RequestUri), webException);
                case WebExceptionStatus.SendFailure:
                    return new CommunicationException(Res.GetString(Res.S("HttpSendFailure"), request.RequestUri), webException);
                ...
                case WebExceptionStatus.ProtocolError:
                    ...
                    {
                        return new ServiceActivationException(Res.GetString(Res.S("Hosting_ServiceActivationFailed"), request.RequestUri));
                    }
                    else
                    {
                        return null;
        ...
        public static Exception CreateResponseIOException(IOException ioException, TimeSpan receiveTimeout)
        {
            ...
            return new CommunicationException(Res.GetString(Res.S("HttpTransferError"), ioException.Message), ioException);
        }
        ...
        public static Exception CreateResponseWebException(WebException webException, HttpWebResponse response)
        {
            ...
            {
                case WebExceptionStatus.RequestCanceled:
                    return TraceResponseException(new CommunicationObjectAbortedException(Res.GetString(Res.S("HttpRequestAborted"), response.ResponseUri), webException));
                case WebExceptionStatus.ConnectionClosed:
                    return TraceResponseException(new CommunicationException(webException.Message, webException));
                case WebExceptionStatus.Timeout:
                    return TraceResponseException(new TimeoutException(Res.GetString(Res.S("HttpResponseTimedOut"), response.ResponseUri,
        ...
        public static Exception CreateRequestCanceledException(Exception webException, HttpWebRequest request, HttpAbortReason abortReason)
        {
            switch (abortReason)
            {
                case HttpAbortReason.Aborted:
                    return new CommunicationObjectAbortedException(Res.GetString(Res.S("HttpRequestAborted"), request.RequestUri), webException);
                case HttpAbortReason.TimedOut:
                    return new TimeoutException(CreateRequestTimedOutMessage(request), webException);
                default:
                    return new CommunicationException(Res.GetString(Res.S("HttpTransferError"), webException.Message), webException);
            }
        }
        ...
        public static Exception CreateRequestIOException(IOException ioException, HttpWebRequest request, Exception originalException)
        {
            ...
            return new CommunicationException(Res.GetString(Res.S("HttpTransferError"), exception.Message), exception);
        }

        static string CreateRequestTimedOutMessage(HttpWebRequest request)
        {
            return Res.GetString(Res.S("HttpRequestTimedOut"), request.RequestUri, TimeSpan.FromMilliseconds(request.Timeout));
        }
        ...
        static Exception CreateUnexpectedResponseException(WebException responseException, HttpWebResponse response)
        {
            ...
            return TraceResponseException(
                new ProtocolException(Res.GetString(Res.S("UnexpectedHttpResponseCode"),
        ...
        {
            return TraceResponseException(
                new ProtocolException(Res.GetString(Res.S("NullReferenceOnHttpResponse")), nullReferenceException));
        }
        ...
        static string GetResponseStreamString(HttpWebResponse webResponse, out int bytesRead)
        {
            ...
            byte[] responseBuffer = new byte[checked((int)bufferSize)];
            ...
            WebException responseException, HttpChannelFactory<IRequestChannel> factory)
        {
            ...
            {
                string message = Res.GetString(Res.S("HttpAuthorizationFailed"), factory.AuthenticationScheme,
            ...
            {
                string message = Res.GetString(Res.S("HttpAuthorizationForbidden"), factory.AuthenticationScheme);
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    TraceResponseException(new MessageSecurityException(message, responseException)));
            }
			...
			{
				throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
					TraceResponseException(new SecurityNegotiationException(Res.GetString(Res.S("HttpMutualAuthNotSatisfied")),
					responseException)));
			}
		}
        ...
        //        {
        //            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(
        //                Res.S("DigestExplicitCredsImpersonationLevel"), impersonationLevel)));
        //        }
        //    }
        //}
            ...
            HttpChannelFactory<IRequestChannel> factory, WebException responseException, ChannelBinding channelBinding)
        {
            ...
            if ((200 <= (int)response.StatusCode && (int)response.StatusCode < 300) || response.StatusCode == HttpStatusCode.InternalServerError)
            {
                if (response.StatusCode == HttpStatusCode.InternalServerError
                    && string.Compare(response.StatusDescription, HttpChannelUtilities.StatusDescriptionStrings.HttpStatusServiceActivationException, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ServiceActivationException(Res.GetString(Res.S("Hosting_ServiceActivationFailed"), request.RequestUri)));
                }
                else
                {
					// net std
                            ...
                            if (!ValidateEmptyContent(response))
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(TraceResponseException(
                                    new ProtocolException(
                                        Res.GetString(Res.S("HttpContentTypeHeaderRequired")),
                                        responseException)));
                            }
                        }
                        else if (response.ContentLength != 0)
                        {
                            ...
                            if (!encoder.IsContentTypeSupported(response.ContentType))
                            {
                                ...
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(TraceResponseException(
                                    new ProtocolException(
                                        Res.GetString(
                                            Res.S("ResponseContentTypeMismatch"),
            ...
            if (this.preReadBuffer != null)
            {
                ...
                if (offset >= buffer.Length)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("offset", offset,
                        Res.GetString(Res.S("OffsetExceedsBufferBound"), buffer.Length - 1)));
                }
