	...
	public abstract class BodyWriter
    {
        ...
        public BodyWriter CreateBufferedCopy(int maxBufferSize)
        {
            ...
            else
            {
                ...
                {
                    if (!this.canWrite)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("BodyWriterCanOnlyBeWrittenOnce"))));
                    this.canWrite = false;
                }
                BodyWriter bodyWriter = OnCreateBufferedCopy(maxBufferSize);
                if (!bodyWriter.IsBuffered)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("BodyWriterReturnedIsNotBuffered"))));
                return bodyWriter;
            }
        }
