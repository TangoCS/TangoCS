...
namespace System.ServiceModel.Channels
{
    ...
    class XmlObjectSerializerHeader : MessageHeader
    {
            ...
            : this(serializer, mustUnderstand, actor, relay)
        {
            ...
            if (name.Length == 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("SFXHeaderNameCannotBeNullOrEmpty")), "name"));
            }
        ...
        public override bool IsMessageVersionSupported(MessageVersion messageVersion)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("EnvelopeVersionUnknown"), messageVersion.Envelope.ToString())));
            }
        }
    ...
    abstract class ReadableMessageHeader : MessageHeader
    {
        ...
        protected override void OnWriteStartHeader(XmlDictionaryWriter writer, MessageVersion messageVersion)
        {
            if (!IsMessageVersionSupported(messageVersion))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("MessageHeaderVersionNotSupported"), this.GetType().FullName, messageVersion.ToString()), "version"));
