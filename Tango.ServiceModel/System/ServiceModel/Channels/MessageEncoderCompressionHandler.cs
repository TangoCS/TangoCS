...
namespace System.ServiceModel.Channels
{
    ...
    internal static class MessageEncoderCompressionHandler
    {
        ...
        internal static void DecompressBuffer(ref ArraySegment<byte> buffer, BufferManager bufferManager, CompressionFormat compressionFormat, long maxReceivedMessageSize)
        {
            ...
            using (BufferManagerOutputStream bufferedOutStream = new BufferManagerOutputStream(Res.S("MaxReceivedMessageSizeExceeded"), 1024, maxDecompressedSize, bufferManager))
            {
        ...
        {
            using (BufferManagerOutputStream bufferedOutStream = new BufferManagerOutputStream(Res.S("MaxSentMessageSizeExceeded"), 1024, int.MaxValue, bufferManager))
            {
                bufferedOutStream.Write(buffer.Array, 0, buffer.Offset);
