    ...
    static class DnsCache
    {
                    ...
                    lock (ThisLock)
                    {
                        if (machineName == null)
                        {
                            ...
                            catch
                            {
								...
								machineName = Environment.MachineName; //UnsafeNativeMethods.GetComputerName(ComputerNameFormat.PhysicalNetBIOS);
                            }
                        }
                    }
                }
        ...
        public static IPHostEntry Resolve(Uri uri)
        {
                    ...
                    else
                    {
                        if (cacheEntry.HostEntry == null)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                new EndpointNotFoundException(Res.GetString(Res.S("DnsResolveFailed"), hostName)));
                        }
            ...
            if (hostEntry == null)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new EndpointNotFoundException(Res.GetString(Res.S("DnsResolveFailed"), hostName), dnsException));
                }
            }
