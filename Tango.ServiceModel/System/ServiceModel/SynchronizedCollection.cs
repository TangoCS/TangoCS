    ...
    public class SynchronizedCollection<T> : IList<T>, IList
    {
                ...
                lock (this.sync)
                {
                    if (index < 0 || index >= this.items.Count)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("index", index,
                                                    Res.GetString(Res.S("ValueMustBeInRange"), 0, this.items.Count - 1)));
        ...
        public void Insert(int index, T item)
        {
            lock (this.sync)
            {
                if (index < 0 || index > this.items.Count)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("index", index,
                                                    Res.GetString(Res.S("ValueMustBeInRange"), 0, this.items.Count)));
        ...
        static void VerifyValueType(object value)
        {
            if (value == null)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("SynchronizedCollectionWrongTypeNull"))));
                }
            }
            else if (!(value is T))
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("SynchronizedCollectionWrongType1"), value.GetType().FullName)));
            }
        }
    }
}
