...
namespace System.ServiceModel
{
        ...
        where TChannel : class
    {
        ...
        protected ClientBase(Binding binding, EndpointAddress remoteAddress)
        {
            ...
            {
                this.endpointTrait = new ProgrammaticEndpointTrait<TChannel>(binding, remoteAddress);
                InitializeChannelFactoryRef();
            }
            else
            {
        ...
        public static CacheSetting CacheSetting
        {
            ...
            set
            {
                lock (cacheLock)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxImmutableClientBaseCacheSetting"), typeof(TChannel).ToString())));
                    }
                    else
                    {
                        cacheSetting = value;
        ...
        //    ProxyOperationRuntime GetOperationByName(string methodName)
        //    {
        //        ProxyOperationRuntime op = this.runtime.GetOperationByName(methodName);
        //        if (op == null)
        //        {
        //            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException(Res.GetString(Res.S("SFxMethodNotSupported1"), methodName)));
        //        }
        //        return op;
        //    }
