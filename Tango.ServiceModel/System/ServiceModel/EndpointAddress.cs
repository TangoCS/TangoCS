    ...
    public class EndpointAddress
    {
        ...
        */
        ...
        internal EndpointAddress(Uri uri, EndpointIdentity identity, AddressHeaderCollection headers, XmlDictionaryReader metadataReader, XmlDictionaryReader extensionReader, XmlDictionaryReader pspReader)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("MultipleIdentities")), "extensionReader"));
            }
        ...
        void Init(AddressingVersion version, Uri uri, EndpointIdentity identity, AddressHeaderCollection headers, XmlBuffer buffer, int metadataSection, int extensionSection, int pspSection)
        {
            if (!uri.IsAbsoluteUri)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("uri", Res.GetString(Res.S("UriMustBeAbsolute")));
            ...
            if (IsAnonymous)
            {
                ...
                else
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new ProtocolException(Res.GetString(Res.S("AddressingVersionNotSupported"), message.Version.Addressing)));
                }
            }
            else if (IsNone)
            {
        ...
        internal static EndpointAddress ReadFrom(XmlDictionaryReader reader, out AddressingVersion version)
        {
            ...
            else if (reader.NodeType != XmlNodeType.Element)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(
                    "reader", Res.GetString(Res.S("CannotDetectAddressingVersion")));
            }
            else
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(
                    "reader", Res.GetString(Res.S("AddressingVersionNotSupported"), reader.NamespaceURI));
            }
        ...
        static EndpointAddress ReadFromDriver(AddressingVersion addressingVersion, XmlDictionaryReader reader)
        {
            ...
            else
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("addressingVersion",
                    Res.GetString(Res.S("AddressingVersionNotSupported"), addressingVersion));
            }
        ...
        internal static XmlBuffer ReadExtensions(XmlDictionaryReader reader, AddressingVersion version, XmlBuffer buffer, out EndpointIdentity identity, out int section)
        {
            ...
            while (reader.IsStartElement())
            {
                ...
                {
                    if (identity != null)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateXmlException(reader, Res.GetString(Res.S("UnexpectedDuplicateElement"), XD.AddressingDictionary.Identity.Value, XD.AddressingDictionary.IdentityExtensionNamespace.Value)));
                    identity = EndpointIdentity.ReadIdentity(reader);
                }
                else if (version != null && reader.NamespaceURI == version.Namespace)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateXmlException(reader, Res.GetString(Res.S("AddressingExtensionInBadNS"), reader.LocalName, reader.NamespaceURI)));
                }
                else
                {
        ...
        static bool ReadContentsFrom200408(XmlDictionaryReader reader, out Uri uri, out AddressHeaderCollection headers, out EndpointIdentity identity, out XmlBuffer buffer, out int metadataSection, out int extensionSection, out int pspSection)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateXmlException(reader, Res.GetString(Res.S("UnexpectedElementExpectingElement"), reader.LocalName, reader.NamespaceURI, XD.AddressingDictionary.Address.Value, XD.Addressing200408Dictionary.Namespace.Value)));
            }
            ...
            // Metadata
            if (reader.IsStartElement(MetadataStrings.MetadataExchangeStrings.Metadata,
            ...
            else
            {
                if (!Uri.TryCreate(address, UriKind.Absolute, out uri))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(Res.GetString(Res.S("InvalidUriValue"), address, XD.AddressingDictionary.Address.Value, AddressingVersion.WSAddressingAugust2004.Namespace)));
            }
            return false;
        }
        ...
        static bool ReadContentsFrom10(XmlDictionaryReader reader, out Uri uri, out AddressHeaderCollection headers, out EndpointIdentity identity, out XmlBuffer buffer, out int metadataSection, out int extensionSection)
        {
            ...
            if (!reader.IsStartElement(XD.AddressingDictionary.Address, XD.Addressing10Dictionary.Namespace))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateXmlException(reader, Res.GetString(Res.S("UnexpectedElementExpectingElement"), reader.LocalName, reader.NamespaceURI, XD.AddressingDictionary.Address.Value, XD.Addressing10Dictionary.Namespace.Value)));
            string address = reader.ReadElementContentAsString();
            ...
            else
            {
                if (!Uri.TryCreate(address, UriKind.Absolute, out uri))
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(Res.GetString(Res.S("InvalidUriValue"), address, XD.AddressingDictionary.Address.Value, XD.Addressing10Dictionary.Namespace.Value)));
                }
            }
            return false;
        }
        ...
        void WriteContentsTo200408(XmlDictionaryWriter writer)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("addressingVersion", Res.GetString(Res.S("SFxNone2004")));
            }
            else
            {
                writer.WriteString(this.Uri.AbsoluteUri);
