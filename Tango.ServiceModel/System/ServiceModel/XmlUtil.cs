...
namespace System.ServiceModel
{
    ...
    static class XmlUtil
    {
        ...
        public static string GetXmlLangAttribute(XmlReader reader)
        {
            ...
            if (xmlLang == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(Res.GetString(Res.S("XmlLangAttributeMissing"))));
        ...
        public static void ParseQName(XmlReader reader, string qname, out string localName, out string ns)
        {
            ...
            else
            {
                if (index == qname.Length - 1)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(Res.GetString(Res.S("InvalidXmlQualifiedName"), qname)));
                prefix = TrimStart(qname.Substring(0, index));
                localName = TrimEnd(qname.Substring(index + 1));
            }
            ns = reader.LookupNamespace(prefix);
            if (ns == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(Res.GetString(Res.S("UnboundPrefixInQName"), qname)));
        }
    }
}
