    ...
    public class FaultReason
    {
        ...
        public FaultReason(IEnumerable<FaultReasonText> translations)
        {
            ...
            foreach (FaultReasonText faultReasonText in translations)
                count++;
            if (count == 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("AtLeastOneFaultReasonMustBeSpecified")), "translations"));
            FaultReasonText[] array = new FaultReasonText[count];
            int index = 0;
            foreach (FaultReasonText faultReasonText in translations)
            {
                if (faultReasonText == null)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("translations", Res.GetString(Res.S("NoNullTranslations")));
        ...
        public FaultReasonText GetMatchingTranslation(CultureInfo cultureInfo)
        {
            ...
            if (translations.Count == 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("NoMatchingTranslationFoundForFaultText"))));
