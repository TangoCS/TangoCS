    ...
    public abstract class SynchronizedKeyedCollection<K, T> : SynchronizedCollection<T>
    {
            ...
            : base(syncRoot)
        {
            ...
            if (dictionaryCreationThreshold < -1)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("dictionaryCreationThreshold", dictionaryCreationThreshold,
                                                    Res.GetString(Res.S("ValueMustBeInRange"), -1, int.MaxValue)));
        ...
        void AddKey(K key, T item)
        {
            ...
            else
            {
                if (this.Contains(key))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("CannotAddTwoItemsWithTheSameKeyToSynchronizedKeyedCollection0"))));
        ...
        protected void ChangeItemKey(T item, K newKey)
        {
            ...
            if (!this.ContainsItem(item))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("ItemDoesNotExistInSynchronizedKeyedCollection0"))));
