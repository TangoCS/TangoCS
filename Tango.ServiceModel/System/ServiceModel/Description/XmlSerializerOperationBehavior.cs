            ...
            internal class OperationReflector
            {
                ...
                internal void EnsureMessageInfos()
                {
                    if (this.request == null)
                    {
                        ...
                        {
                            if (knownType == null)
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxKnownTypeNull"), Operation.Name)));
                        ...
                        if (this.request != null && this.IsRpc && this.Operation.IsValidateRpcWrapperName && this.request.BodyMapping.XsdElementName != this.Operation.Name)
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxRpcMessageBodyPartNameInvalid"), Operation.Name, this.Operation.Messages[0].MessageName, request.BodyMapping.XsdElementName, this.Operation.Name)));
                        if (!this.IsOneWay)
                        {
                            ...
                            if (this.reply != null && this.IsRpc && this.Operation.IsValidateRpcWrapperName && this.reply.BodyMapping.XsdElementName != responseName.EncodedName)
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxRpcMessageBodyPartNameInvalid"), Operation.Name, this.Operation.Messages[1].MessageName, reply.BodyMapping.XsdElementName, responseName.EncodedName)));
                        }
                        ...
                        else if (headersMapping != null)
                        {
                            ...
                            if (headerName != header.Name)
                            {
                                if (message.MessageType != null)
                                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxHeaderNameMismatchInMessageContract"), message.MessageType, header.MemberInfo.Name, header.Name, headerName)));
                                else
                                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxHeaderNameMismatchInOperation"), this.Operation.Name, this.Operation.DeclaringContract.Name, this.Operation.DeclaringContract.Namespace, header.Name, headerName)));
                            }
                            if (headerNs != header.Namespace)
                            {
                                if (message.MessageType != null)
                                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxHeaderNamespaceMismatchInMessageContract"), message.MessageType, header.MemberInfo.Name, header.Namespace, headerNs)));
                                else
                                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxHeaderNamespaceMismatchInOperation"), this.Operation.Name, this.Operation.DeclaringContract.Name, this.Operation.DeclaringContract.Namespace, header.Namespace, headerNs)));
                            }
                ...
                private MessagePartDescription GetWrapperPart(MessageDescription message)
                {
                    if (message.Body.Parts.Count != 1)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxRpcMessageMustHaveASingleBody"), Operation.Name, message.MessageName)));
                    ...
                    if (bodyObjectType.BaseType != null && bodyObjectType.BaseType != typeof(object))
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxBodyObjectTypeCannotBeInherited"), bodyObjectType.FullName)));
                    if (typeof(IEnumerable).IsAssignableFrom(bodyObjectType))
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxBodyObjectTypeCannotBeInterface"), bodyObjectType.FullName, typeof(IEnumerable).FullName)));
                    if (typeof(IXmlSerializable).IsAssignableFrom(bodyObjectType))
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxBodyObjectTypeCannotBeInterface"), bodyObjectType.FullName, typeof(IXmlSerializable).FullName)));
                    return bodyPart;
                }
                ...
                XmlMembersMapping LoadHeadersMapping(MessageDescription message, string mappingKey)
                {
                    ...
                    if (headerCount == 0)
                        return null;
                    if (IsEncoded)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxHeadersAreNotSupportedInEncoded"), message.MessageName)));
                ...
                internal XmlMembersMapping ImportFaultElement(FaultDescription fault, out XmlQualifiedName elementName)
                {
                    ...
                    if (faultElementName == null)
                    {
                        ...
                        if (faultElementName == null)
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxFaultTypeAnonymous"), this.Operation.Name, fault.DetailType.FullName)));
                    }
    ...
    static class XmlSerializerHelper
    {
        ...
        static internal XmlReflectionMember GetXmlReflectionMember(XmlName memberName, XmlName elementName, string ns, Type type, ICustomAttributeProvider additionalAttributesProvider, bool isMultiple, bool isEncoded, bool isWrapped)
        {
            if (isEncoded && isMultiple)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxMultiplePartsNotAllowedInEncoded"), elementName.DecodedName, ns)));
            ...
            if (isEncoded)
            {
                ...
                else
                {
                    ...
                    if (invalidAttributeType != null)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxInvalidSoapAttribute"), invalidAttributeType, elementName.DecodedName)));
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (invalidAttributeType != null)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(isWrapped ? "SFxInvalidXmlAttributeInWrapped" : Res.S("SFxInvalidXmlAttributeInBare"), invalidAttributeType, elementName.DecodedName)));
                    if (member.XmlAttributes.XmlArray != null && isMultiple)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxXmlArrayNotAllowedForMultiple"), elementName.DecodedName, ns)));
