    ...
    internal static class TaskOperationDescriptionValidator
    {
        ...
        private static void EnsureNoSyncMethod(OperationDescription operation)
        {
            if (operation.SyncMethod != null)
            {
                ...
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("CannotHaveTwoOperationsWithTheSameName3"), method1Name, method2Name, operation.DeclaringContract.ContractType)));
            }
        }
        ...
        private static void EnsureParametersAreSupported(MethodInfo method)
        {
            foreach (ParameterInfo parameter in method.GetParameters())
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new NotSupportedException(Res.GetString(Res.S("TaskMethodParameterNotSupported"), parameterType)));
                }
            }
        }
        ...
        private static void EnsureNoOutputParameters(MethodInfo method)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("TaskMethodMustNotHaveOutParameter"))));
            }
        }
    }
}
