    ...
    static class NamingHelper
    {
        ...
        internal static void CheckUriProperty(string ns, string propName)
        {
            ...
            if (!Uri.TryCreate(ns, UriKind.RelativeOrAbsolute, out uri))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(Res.GetString(Res.S("SFXUnvalidNamespaceValue"), ns, propName));
        }
        ...
        internal static void CheckUriParameter(string ns, string paramName)
        {
            Uri uri;
            if (!Uri.TryCreate(ns, UriKind.RelativeOrAbsolute, out uri))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument(paramName, Res.GetString(Res.S("SFXUnvalidNamespaceParam"), ns));
        }
    ...
    static internal class ServiceReflector
    {
        ...
        static internal List<Type> GetInterfaces(Type service)
        {
            ...
            if (!implicitContract)
            {
                ...
                if (t != null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxContractInheritanceRequiresInterfaces2"), service, t)));
                }
                foreach (MethodInfo method in GetMethodsInternal(service))
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("ServicesWithoutAServiceContractAttributeCan2"), operationContractProviderType.Name, method.Name, service.FullName)));
                    }
                }
            }
            foreach (Type t in service.GetInterfaces())
            {
                if (t.IsDefined(typeof(ServiceContractAttribute), false))
                {
                    if (implicitContract)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxContractInheritanceRequiresInterfaces"), service, t)));
                    }
                    types.Add(t);
                }
            }
            ...
            catch (Exception e)
            {
                ...
                if (type != null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        Res.GetString(Res.S("SFxErrorReflectingOnType2"), attrType.Name, type.Name), e));
                }
                else if (method != null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        Res.GetString(Res.S("SFxErrorReflectingOnMethod3"),
                ...
                else if (param != null)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            Res.GetString(Res.S("SFxErrorReflectingOnParameter4"),
                                         attrType.Name, param.Name, method.Name, method.ReflectedType.Name), e));
                    }
                }
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                    Res.GetString(Res.S("SFxErrorReflectionOnUnknown1"), attrType.Name), e));
            }
        }
            ...
            where T : class
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("tooManyAttributesOfTypeOn2"), attrType, attrProvider.ToString())));
            }
            else
            {
                return attrs[0] as T;
            ...
            where T : class
        {
            ...
            if (result == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("couldnTFindRequiredAttributeOfTypeOn2"), typeof(T), attrProvider.ToString())));
            }
            return result;
        }
#else
        static internal object GetRequiredSingleAttribute(Type attrType, ICustomAttributeProvider attrProvider)
        {
            ...
            if (result == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("couldnTFindRequiredAttributeOfTypeOn2"), attrType, attrProvider.ToString())));
            }
            return result;
        }
#endif
            ...
            where T : class
        {
            ...
            if (result != null)
            {
                ...
                foreach (Type otherType in attrTypeGroup)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxDisallowedAttributeCombination"), attrProvider, attrType.FullName, otherType.FullName)));
                    }
                }
            }
            return result;
        ...
        static internal Type GetContractTypeAndAttribute(Type interfaceType, out ServiceContractAttribute contractAttribute)
        {
            ...
            if (types.Count == 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("AttemptedToGetContractTypeForButThatTypeIs1"), interfaceType.Name)));
            }
            ...
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                Res.GetString(Res.S("SFxNoMostDerivedContract"), interfaceType.Name)));
        }
        ...
        static internal void ValidateParameterMetadata(MethodInfo methodInfo)
        {
            ...
            foreach (ParameterInfo parameter in parameters)
            {
                if (!parameter.ParameterType.IsByRef)
                {
                    if (parameter.IsOut)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(Res.GetString(Res.S("SFxBadByValueParameterMetadata"),
                ...
                else
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(Res.GetString(Res.S("SFxBadByReferenceParameterMetadata"),
                            methodInfo.Name, methodInfo.DeclaringType.Name)));
                    }
                }
            }
        ...
        static MethodInfo GetEndMethodInternal(MethodInfo beginMethod)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("NoEndMethodFoundForAsyncBeginMethod3"), beginMethod.Name, beginMethod.DeclaringType.FullName, endMethodName)));
            }
            if (endMethods.Length > 1)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("MoreThanOneEndMethodFoundForAsyncBeginMethod3"), beginMethod.Name, beginMethod.DeclaringType.FullName, endMethodName)));
            }
        ...
        static internal MethodInfo GetEndMethod(MethodInfo beginMethod)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("InvalidAsyncEndMethodSignatureForMethod2"), endMethod.Name, endMethod.DeclaringType.FullName)));
            }
        ...
        static internal bool IsBegin(OperationContractAttribute opSettings, MethodInfo method)
        {
            if (opSettings.AsyncPattern)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("InvalidAsyncBeginMethodSignatureForMethod2"), method.Name, method.DeclaringType.FullName)));
                }
