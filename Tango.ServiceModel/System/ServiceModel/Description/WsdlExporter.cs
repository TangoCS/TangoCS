...
namespace System.ServiceModel.Description
{
    ...
    public class WsdlExporter : MetadataExporter
    {
        ...
        {
            if (this.isFaulted)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("WsdlExporterIsFaulted"))));
            ...
            if (!this.exportedContracts.ContainsKey(contract))
            {
                try
                {
                    ...
                    foreach (OperationDescription operation in contract.Operations)
                    {
                        ...
                        {
                            string warningMsg = isWildcardAction ? Res.GetString(Res.S("WarnSkippingOpertationWithWildcardAction"), contract.Name, contract.Namespace, operation.Name)
                                : Res.GetString(Res.S("WarnSkippingOpertationWithSessionOpenNotificationEnabled"), "Action", OperationDescription.SessionOpenedAction, contract.Name, contract.Namespace, operation.Name);
        ...
        void ExportEndpoint(ServiceEndpoint endpoint, XmlQualifiedName wsdlServiceQName, BindingParameterCollection bindingParameters)
        {
            if (endpoint.Binding == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("EndpointsMustHaveAValidBinding1"), endpoint.Name)));
            ...
            try
            {
                ...
                if (bindingNameWasUniquified)
                    Errors.Add(new MetadataConversionError(Res.GetString(Res.S("WarnDuplicateBindingQNameNameOnExport"), endpoint.Binding.Name, endpoint.Binding.Namespace, endpoint.Contract.Name), true /*isWarning*/));
            }
            catch
            {
                isFaulted = true;
        ...
        WsdlNS.PortType CreateWsdlPortType(ContractDescription contract)
        {
            ...
            if (wsdl.PortTypes[wsdlPortType.Name] != null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("DuplicateContractQNameNameOnExport"), contract.Name, contract.Namespace)));
        ...
        internal static class WSAddressingHelper
        {
            ...
            internal static void AddWSAddressingAssertion(MetadataExporter exporter, PolicyConversionContext context, AddressingVersion addressVersion)
            {
                    ...
                    else
                    {
                        ...
                        if (exporter.State.ContainsKey(key) && exporter.State[key] is SupportedAddressingMode)
                        {
                            ...
                            if (!SupportedAddressingModeHelper.IsDefined(mode))
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SupportedAddressingModeNotSupported"), mode)));
                        }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(Res.GetString(Res.S("AddressingVersionNotSupported"), addressVersion)));
                }
        ...
        Exception ThrowExtensionException(ContractDescription contract, IWsdlExportExtension exporter, Exception e)
        {
            ...
            string errorMessage = Res.GetString(Res.S("WsdlExtensionContractExportError"), exporter.GetType(), contractIdentifier);
        ...
        Exception ThrowExtensionException(ServiceEndpoint endpoint, IWsdlExportExtension exporter, Exception e)
        {
            ...
            string errorMessage = Res.GetString(Res.S("WsdlExtensionEndpointExportError"), exporter.GetType(), endpointIdentifier);
