    ...
    static class SchemaHelper
    {
        ...
        static internal void AddElementToSchema(XmlSchemaElement element, XmlSchema schema, XmlSchemaSet schemaSet)
        {
            ...
            if (existingElement != null)
            {
                if (element.SchemaType == existingElement.SchemaType && element.SchemaTypeName == existingElement.SchemaTypeName)
                    return;
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxConflictingGlobalElement"), element.Name, schema.TargetNamespace, GetTypeName(element), GetTypeName(existingElement))));
            }
        ...
        static internal void AddTypeToSchema(XmlSchemaType type, XmlSchema schema, XmlSchemaSet schemaSet)
        {
            ...
            if (existingType != null)
            {
                ...
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxConflictingGlobalType"), type.Name, schema.TargetNamespace)));
            }
        ...
        static internal void HandleSchemaValidationError(object sender, ValidationEventArgs args, Collection<MetadataConversionError> errors)
        {
            ...
            if (args.Exception != null && args.Exception.SourceUri != null)
            {
                ...
                warning = new MetadataConversionError(Res.GetString(Res.S("SchemaValidationError"), ex.SourceUri, ex.LineNumber, ex.LinePosition, ex.Message));
            }
            else
            {
                warning = new MetadataConversionError(Res.GetString(Res.S("GeneralSchemaValidationError"), args.Message));
            }
