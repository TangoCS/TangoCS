...
namespace System.ServiceModel.Description
{
    ...
    abstract class MessageContractExporter
    {
        ...
        void AddElementToSchema(XmlSchemaElement element, string elementNs, XmlSchemaSet schemaSet)
        {
            ...
            if (parentOperation.OperationMethod != null)
            {
                ...
                if (ExportedMessages.ElementTypes.TryGetValue(qname, out existingElement))
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("CannotHaveTwoOperationsWithTheSameElement5"), parentOperation.OperationMethod.DeclaringType, parentOperation.OperationMethod.Name, qname, existingElement.Operation.OperationMethod.DeclaringType, existingElement.Operation.Name)));
                    }
                    return;
                }
                else
        ...
        void ExportAnyMessage(WsdlNS.Message message, MessagePartDescription part)
        {
            ...
            XmlSchema schema = SchemaHelper.GetSchema(GenericMessageTypeName.Namespace, schemas);

            if (!schema.SchemaTypes.Contains(GenericMessageTypeName))
            {
                XmlSchemaComplexType genericMessageType = new XmlSchemaComplexType();
                genericMessageType.Name = GenericMessageTypeName.Name;
            ...
            WsdlNS.MessagePart wsdlPart = AddMessagePart(message, partName, XmlQualifiedName.Empty, GenericMessageTypeName);
        ...
        {
            XmlSchemaSet schemas = this.exporter.GeneratedXmlSchemas;
            XmlSchema schema = SchemaHelper.GetSchema(StreamBodyTypeName.Namespace, schemas);
            if (!schema.SchemaTypes.Contains(StreamBodyTypeName))
            {
                XmlSchemaSimpleType streamBodyType = new XmlSchemaSimpleType();
                streamBodyType.Name = StreamBodyTypeName.Name;
            ...
            ExportMessagePart(message, streamPart, StreamBodyTypeName, null/*xsdType*/, false/*isOptional*/, false/*isNillable*/, skipSchemaExport, !isRpc, wrapperNs, wrapperSequence, schemas);
        }
        ...
        XmlQualifiedName ExportFaultElement(FaultDescription fault)
        {
            ...
            if (XmlName.IsNullOrEmpty(fault.ElementName))
            {
                ...
                if (elementName == null)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxFaultTypeAnonymous"), operation.Name, fault.DetailType.FullName)));
            }
            else
        ...
        protected XmlQualifiedName ExportType(Type type, string partName, string operationName, out XmlSchemaType xsdType)
        {
            xsdType = null;
            if (type == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxExportMustHaveType"), operationName, partName)));
            if (type == typeof(void))
                return null;
        ...
        static protected WsdlNS.MessagePart AddMessagePart(WsdlNS.Message message, string partName, XmlQualifiedName elementName, XmlQualifiedName typeName)
        {
            if (message.Parts[partName] != null)
            {
                if (IsNullOrEmpty(elementName))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("SFxPartNameMustBeUniqueInRpc"), partName)));
                int i = 1;
                while (message.Parts[partName + i] != null)
                {
                    if (i == Int32.MaxValue)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("SFxTooManyPartsWithSameName"), partName)));
                    i++;
                }
        ...
        protected bool CreateMessage(MessageDescription message, int messageIndex, out WsdlNS.Message wsdlMessage)
        {
            ...
            if (ExportedMessages.WsdlMessages.ContainsKey(new MessageDescriptionDictionaryKey(contractContext.Contract, message)))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("MultipleCallsToExportContractWithSameContract"))));
        ...
        protected void ExportMessagePart(WsdlNS.Message message, MessagePartDescription part, XmlQualifiedName typeName, XmlSchemaType xsdType, bool isOptional, bool isNillable, bool skipSchemaExport, bool generateElement, string wrapperNs, XmlSchemaSequence wrapperSequence, XmlSchemaSet schemaSet)
        {
            ...
            else
            {
                if (String.IsNullOrEmpty(typeName.Name))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxAnonymousTypeNotSupported"), message.Name, partName)));
