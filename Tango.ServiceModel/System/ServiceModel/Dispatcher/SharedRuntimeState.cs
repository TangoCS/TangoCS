    ...
    class SharedRuntimeState
    {
        ...
        internal void ThrowIfImmutable()
        {
            if (this.isImmutable)
            {
                if (this.IsOnServer)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxImmutableServiceHostBehavior0"))));
                }
                else
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxImmutableChannelFactoryBehavior0"))));
                }
            }
        }
    }
