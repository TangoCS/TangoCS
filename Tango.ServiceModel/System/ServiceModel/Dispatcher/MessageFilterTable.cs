...
namespace System.ServiceModel.Dispatcher
{
    ...
    public class MessageFilterTable<TFilterData> : IMessageFilterTable<TFilterData>
    {
        ...
        public void Add(MessageFilter filter, TFilterData data, int priority)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgument("filter", Res.GetString(Res.S("FilterExists")));
            }
            ...
            if (this.filterTypeMappings.TryGetValue(filterType, out tableType))
            {
                ...
                if (table == null)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("FilterTableTypeMismatch"))));
                    }
        ...
        public int GetPriority(MessageFilter filter)
        {
            ...
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperCritical(new InvalidOperationException(Res.GetString(Res.S("FilterTableInvalidForLookup"))));
        }
        ...
        public bool GetMatchingValue(Message message, out TFilterData data)
        {
            ...
            for (int i = 0; i < this.tables.Count; ++i)
            {
                ...
                if (this.tables[i].table.GetMatchingValue(message, out currentData))
                {
                    if (dataSet)
                    {
                        throw TraceUtility.ThrowHelperError(new MultipleFilterMatchesException(Res.GetString(Res.S("FilterMultipleMatches")), null, null), message);
                    }
        ...
        internal bool GetMatchingValue(MessageBuffer buffer, Message messageToReadHeaders, out TFilterData data)
        {
            ...
            for (int i = 0; i < this.tables.Count; ++i)
            {
                ...
                if (result)
                {
                    if (dataSet)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MultipleFilterMatchesException(Res.GetString(Res.S("FilterMultipleMatches")), null, null));
                    }
            ...
            for (int i = 0; i < this.tables.Count; ++i)
            {
                ...
                if (this.tables[i].table.GetMatchingFilter(message, out f))
                {
                    ...
                    else
                    {
                        ...
                        c.Add(filter);
                        c.Add(f);
                        throw TraceUtility.ThrowHelperError(new MultipleFilterMatchesException(Res.GetString(Res.S("FilterMultipleMatches")), null, c), message);
                    }
                }
            }
            ...
            for (int i = 0; i < this.tables.Count; ++i)
            {
                ...
                if (this.tables[i].table.GetMatchingFilter(buffer, out f))
                {
                    ...
                    else
                    {
                        ...
                        c.Add(filter);
                        c.Add(f);
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MultipleFilterMatchesException(Res.GetString(Res.S("FilterMultipleMatches")), null, c));
                    }
                }
            }
        ...
        void ValidateTable(IMessageFilterTable<TFilterData> table)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("FilterBadTableType"))));
            }
        }
