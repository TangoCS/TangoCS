            ...
            foreach (KeyValuePair<MessageFilter, FilterData> item in this.filters)
            {
                if (item.Key.Match(message))
                {
                    if (dataSet)
                    {
                        ...
                        f.Add(filter);
                        f.Add(item.Key);
                        throw TraceUtility.ThrowHelperError(new MultipleFilterMatchesException(Res.GetString(Res.S("FilterMultipleMatches")), null, f), message);
                    }
            ...
            foreach (KeyValuePair<MessageFilter, FilterData> item in this.filters)
            {
                if (item.Key.Match(buffer))
                {
                    if (dataSet)
                    {
                        ...
                        f.Add(filter);
                        f.Add(item.Key);
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MultipleFilterMatchesException(Res.GetString(Res.S("FilterMultipleMatches")), null, f));
                    }
