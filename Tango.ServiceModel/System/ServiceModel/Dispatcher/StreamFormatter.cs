        ...
        static MessagePartDescription ValidateAndGetStreamPart(MessageDescription messageDescription, bool isRequest, string operationName)
        {
            ...
            if (HasStream(messageDescription))
            {
                if (messageDescription.IsTypedMessage)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxInvalidStreamInTypedMessage"), messageDescription.MessageName)));
                else if (isRequest)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxInvalidStreamInRequest"), operationName)));
                else
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(Res.GetString(Res.S("SFxInvalidStreamInResponse"), operationName)));
            }
            return null;
        }
        ...
        internal class MessageBodyStream : Stream
        {
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                ...
                if (offset < 0)
                    throw TraceUtility.ThrowHelperError(new ArgumentOutOfRangeException("offset", offset,
                                                    Res.GetString(Res.S("ValueMustBeNonNegative"))), this.message);
                if (count < 0)
                    throw TraceUtility.ThrowHelperError(new ArgumentOutOfRangeException("count", count,
                                                    Res.GetString(Res.S("ValueMustBeNonNegative"))), this.message);
                if (buffer.Length - offset < count)
                    throw TraceUtility.ThrowHelperError(new ArgumentException(Res.GetString(Res.S("SFxInvalidStreamOffsetLength"), offset + count)), this.message);
                ...
                catch (Exception ex)
                {
                    if (Fx.IsFatal(ex))
                        throw;
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new IOException(Res.GetString(Res.S("SFxStreamIOException")), ex));
                }
            }
            ...
            private void EnsureStreamIsOpen()
            {
                if (message.State == MessageState.Closed)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ObjectDisposedException(Res.GetString(
                        isRequest ? "SFxStreamRequestMessageClosed" : Res.S("SFxStreamResponseMessageClosed"))));
            }
