		...
		internal bool EnableFaults {
			...
			set {
				lock (this.ThisLock)
				{
					if (this.IsOnServer)
					{
						string text = Res.GetString(Res.S("SFxSetEnableFaultsOnChannelDispatcher0"));
        ...
        public bool ManualAddressing
        {
            ...
            set
            {
                lock (this.ThisLock)
                {
                    if (this.IsOnServer)
                    {
                        string text = Res.GetString(Res.S("SFxSetManualAddresssingOnChannelDispatcher0"));
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(text));
                    }
                    else
                    {
            ...
            get
            {
                lock (this.ThisLock)
                {
                    ...
                    for (int i = 0; i < this.operations.Count; i++)
                        max = Math.Max(max, this.operations[i].ParameterInspectors.Count);
