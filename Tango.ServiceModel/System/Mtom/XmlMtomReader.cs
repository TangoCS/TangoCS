...
namespace System.Xml
{
	...
	class XmlMtomReader : XmlDictionaryReader, IXmlLineInfo, IXmlMtomReaderInitializer
	{
		...
		internal static void DecrementBufferQuota(int maxBuffer, ref int remaining, int size)
		{
			...
			{
				remaining = 0;
				throw new XmlException("MtomBufferQuotaExceeded");
			}
			else
			{
				remaining -= size;
		...
		void CheckContentType(string contentType)
		{
			if (contentType != null && contentType.Length == 0)
				throw new ArgumentException("MtomContentTypeInvalid");
		}
		...
		void Initialize(Stream stream, string contentType, XmlDictionaryReaderQuotas quotas, int maxBufferSize)
		{
			...
			if (contentType == null)
			{
				...
				if (stream == null)
					throw new XmlException("MtomMessageInvalidContent");
			}
			else
			{
		...
		void ReadMessageMimeVersionHeader(MimeVersionHeader header)
		{
			if (header != null && header.Version != MimeVersionHeader.Default.Version)
				throw new XmlException("MtomMessageInvalidMimeVersion, header.Version");
		}

		void ReadMessageContentTypeHeader(ContentTypeHeader header, out string boundary, out string start, out string startInfo)
		{
			if (header == null)
				throw new XmlException("MtomMessageContentTypeNotFound");
			...
			if (String.Compare(MtomGlobals.MediaType, header.MediaType, StringComparison.OrdinalIgnoreCase) != 0
				...
				throw new XmlException("MtomMessageNotMultipart, MtomGlobals.MediaType");

			string type;
			if (!header.Parameters.TryGetValue(MtomGlobals.TypeParam, out type) || MtomGlobals.XopType != type)
				throw new XmlException("MtomMessageNotApplicationXopXml");

			if (!header.Parameters.TryGetValue(MtomGlobals.BoundaryParam, out boundary))
				throw new XmlException("MtomMessageRequiredParamNotSpecified");
			if (!MailBnfHelper.IsValidMimeBoundary(boundary))
				throw new XmlException("MtomBoundaryInvalid");
		...
		{
			if (header == null)
				throw new XmlException("MtomRootContentTypeNotFound");
			...
			if (String.Compare(MtomGlobals.XopMediaType, header.MediaType, StringComparison.OrdinalIgnoreCase) != 0
				...
				throw new XmlException("MtomRootNotApplicationXopXml, MtomGlobals.XopMediaType");

			string charset;
			if (!header.Parameters.TryGetValue(MtomGlobals.CharsetParam, out charset)
				|| charset == null || charset.Length == 0)
				throw new XmlException("MtomRootRequiredParamNotSpecified");
			...
			if (encoding == null)
			{
				...
				if (encoding == null)
				{
					...
					throw new XmlException("MtomRootUnexpectedCharset, charset");
				}
			}
			...
			if (expectedType != null)
			{
				...
				if (rootType != expectedType)
					throw new XmlException("MtomRootUnexpectedType, rootType");
			}
		...
		void CheckContentTransferEncodingOnRoot(ContentTransferEncodingHeader header)
		{
			if (header != null && header.ContentTransferEncoding == ContentTransferEncoding.Other)
				throw new XmlException("MtomContentTransferEncodingNotSupported");
		}

		void CheckContentTransferEncodingOnBinaryPart(ContentTransferEncodingHeader header)
		{
			if (header == null)
				throw new XmlException("MtomContentTransferEncodingNotPresent");
			else if (header.ContentTransferEncoding != ContentTransferEncoding.Binary)
				throw new XmlException("MtomInvalidTransferEncodingForMimePart");
		}
		...
		string GetStartUri(string startUri)
		{
			if (startUri.StartsWith("<", StringComparison.Ordinal))
			{
				if (startUri.EndsWith(">", StringComparison.Ordinal))
					...
					throw new XmlException("MtomInvalidStartUri");
			}
			else
				...
				if (xmlReader.IsStartElement(MtomGlobals.XopIncludeLocalName, MtomGlobals.XopIncludeNamespace))
				{
					...
					while (xmlReader.MoveToNextAttribute())
					{
						if (xmlReader.LocalName == MtomGlobals.XopIncludeHrefLocalName && xmlReader.NamespaceURI == MtomGlobals.XopIncludeHrefNamespace)
							...
							throw new XmlException("MtomXopIncludeInvalidXopAttributes");
					}
					if (uri == null)
						throw new XmlException("MtomXopIncludeHrefNotSpecified");
					...
					else
					{
						...
						while (xmlReader.Depth > xopDepth)
						{
							if (xmlReader.IsStartElement() && xmlReader.NamespaceURI == MtomGlobals.XopIncludeNamespace)
								throw new XmlException("MtomXopIncludeInvalidXopElement");
		...
		MimePart ReadMimePart(string uri)
		{
			...
			if (uri == null || uri.Length == 0)
				throw new XmlException("MtomInvalidEmptyURI");
			...
			if (contentID == null)
				throw new XmlException("MtomInvalidCIDUri");

			if (mimeParts != null && mimeParts.TryGetValue(contentID, out part))
			{
				if (part.ReferencedFromInfoset)
					throw new XmlException("MtomMimePartReferencedMoreThanOnce");
			}
			else
			{
				...
				while (part == null && mimeReader.ReadNextPart())
				{
					...
					if (contentStream == null)
						throw new XmlException("MtomMessageInvalidContentInMimePart");
					...
					if (mimeParts.Count > maxMimeParts)
						throw new XmlException("MaxMimePartsExceeded");
				...
				if (part == null)
					throw new XmlException("MtomPartNotFound");
			}
		...
		MimePart ReadRootMimePart()
		{
			...
			if (!mimeReader.ReadNextPart())
				throw new XmlException("MtomRootPartNotFound");
		...
		internal class XopIncludeReader : XmlDictionaryReader, IXmlLineInfo
		{
			...
			public override int ReadValueAsBase64(byte[] buffer, int offset, int count)
			{
				...
				if (offset > buffer.Length)
					throw new ArgumentOutOfRangeException("offset", "OffsetExceedsBufferSize");
				if (count < 0)
					throw new ArgumentOutOfRangeException("count", "ValueMustBeNonNegative");
				if (count > buffer.Length - offset)
					throw new ArgumentOutOfRangeException("count", "SizeExceedsRemainingBufferSpace");
			...
			public override string ReadContentAsString()
			{
				...
				do
				{
					...
					if (val.Length > stringContentQuota)
						throw new Exception ("Quotas.MaxStringContentLength");
	...
	internal class MimeMessageReader
	{
		...
		public Stream GetContentStream()
		{
			if (getContentStreamCalled)
				throw new InvalidOperationException("MimeMessageGetContentStreamCalledAlready");
	...
	internal class MimeReader
	{
		...
		public bool ReadNextPart()
		{
			...
			throw new FormatException("MimeReaderTruncated");
		}
	...
	internal class DelimittedStreamReader
	{
		...
		class DelimittedReadStream : Stream
		{
			...
			{
				get { throw new NotSupportedException("SeekNotSupportedOnStream"); }
			}
			...
			public override long Position
			{
				get
				{
					throw new NotSupportedException("SeekNotSupportedOnStream");
				}
				set { throw new NotSupportedException("SeekNotSupportedOnStream"); }
			}

			public override IAsyncResult BeginWrite(byte[] buffer, int offset, int count, AsyncCallback callback, object state)
			{
				throw new NotSupportedException("WriteNotSupportedOnStream");
			}
	...
	internal class MimeHeaders
	{
		...
		public void Add(MimeHeader header)
		{
			...
			if (headers.TryGetValue(header.Name, out existingHeader))
				throw new FormatException("MimeReaderHeaderAlreadyExists");
			else
		...
		void ParseValue()
		{
			if (this.parameters == null)
			{
				...
				if (offset >= Value.Length || Value[offset++] != '/')
					throw new FormatException("MimeContentTypeHeaderInvalid");
	...
	internal class MimeVersionHeader : MimeHeader
	{
		...
		void ParseValue()
		{
			...
			else
			{
				...
				if (!MailBnfHelper.SkipCFWS(Value, ref offset))
					throw new FormatException("MimeVersionHeaderInvalid");
	...
	internal class MimeHeaderReader
	{
								...
								else
								{
									...
									else if (*ptr < 33 || *ptr > 126)
									{
										if (name == null && *ptr == (byte)'\r')
										{
											ptr++;
											if (ptr >= end || *ptr != '\n')
												throw new FormatException("MimeReaderMalformedHeader");
											goto case ReadState.EOF;
										}

										throw new FormatException("MimeHeaderInvalidCharacter, (char)(*ptr");
									}
								}
							}
							AppendName(new string((sbyte*)start, 0, (int)(ptr - start)), maxBuffer, ref remaining);
		...
		public void Reset(Stream stream)
		{
			if (readState != ReadState.EOF)
				throw new InvalidOperationException("MimeReaderResetCalledBeforeEOF");
	...
	internal class BufferedReadStream : Stream
	{
		...
		{
			if (!CanRead)
				throw new NotSupportedException("ReadNotSupportedOnStream");
	...
	internal static class MailBnfHelper
	{
		...
		public static bool SkipCFWS(string data, ref int offset)
		{
			...
			{
				if (data[offset] > 127)
					throw new FormatException("MimeHeaderInvalidCharacter");
