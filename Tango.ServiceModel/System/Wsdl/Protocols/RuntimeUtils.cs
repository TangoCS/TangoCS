    ...
    internal class RuntimeUtils {
        ...
        static void OnUnknownAttribute(object sender, XmlAttributeEventArgs e) {
            ...
            if (e.ExpectedAttributes == null)
                throw new InvalidOperationException(Res.GetString(Res.S("WebUnknownAttribute"), e.Attr.Name, e.Attr.Value));
            else if (e.ExpectedAttributes.Length == 0)
                throw new InvalidOperationException(Res.GetString(Res.S("WebUnknownAttribute2"), e.Attr.Name, e.Attr.Value));
            else
                throw new InvalidOperationException(Res.GetString(Res.S("WebUnknownAttribute3"), e.Attr.Name, e.Attr.Value, e.ExpectedAttributes));
        }
        ...
        internal static void OnUnknownElement(object sender, XmlElementEventArgs e) {
            ...
            if (e.ExpectedElements == null)
                throw new InvalidOperationException(Res.GetString(Res.S("WebUnknownElement"), xml));
            else if (e.ExpectedElements.Length == 0)
                throw new InvalidOperationException(Res.GetString(Res.S("WebUnknownElement1"), xml));
            else
                throw new InvalidOperationException(Res.GetString(Res.S("WebUnknownElement2"), xml, e.ExpectedElements));
        }
