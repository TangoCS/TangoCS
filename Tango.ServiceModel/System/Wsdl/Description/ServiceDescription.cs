...
namespace System.Web.Services.Description {
    ...
    public sealed class ServiceDescription : NamedItem {
        ...
        private static void InstanceValidation (object sender, ValidationEventArgs args) {
            warnings.Add(Res.GetString(Res.S("WsdlInstanceValidationDetails"), args.Message, args.Exception.LineNumber.ToString(CultureInfo.InvariantCulture), args.Exception.LinePosition.ToString(CultureInfo.InvariantCulture)));
        }
    ...
    public sealed class Message : NamedItem {
        ...
        public MessagePart FindPartByName(string partName) {
            ...
            throw new ArgumentException(Res.GetString(Res.S("MissingMessagePartForMessageFromNamespace3"), partName, Name, ServiceDescription.TargetNamespace), "partName");
        }
    }
    ...
    public sealed class ServiceDescriptionFormatExtensionCollection : ServiceDescriptionBaseCollection {
        ...
        protected override void OnValidate(object value) {
            if (!(value is XmlElement || value is ServiceDescriptionFormatExtension)) 
                throw new ArgumentException(Res.GetString(Res.S("OnlyXmlElementsOrTypesDerivingFromServiceDescriptionFormatExtension0")), "value");
    ...
    public sealed class OperationMessageCollection : ServiceDescriptionBaseCollection {
        ...
        protected override void OnInsert(int index, object value) {
            if (Count > 1 || (Count == 1 && value.GetType() == List[0].GetType()))
                throw new InvalidOperationException(Res.GetString(Res.S("WebDescriptionTooManyMessages")));
        ...
        protected override void OnSet(int index, object oldValue, object newValue) {
            if (oldValue.GetType() != newValue.GetType()) throw new InvalidOperationException(Res.GetString(Res.S("WebDescriptionTooManyMessages")));
        ...
        protected override void OnValidate(object value) {
            if (!(value is OperationInput || value is OperationOutput))
                throw new ArgumentException(Res.GetString(Res.S("OnlyOperationInputOrOperationOutputTypes")), "value");
            base.OnValidate(value);
        }
    }
    ...
    public abstract class ServiceDescriptionBaseCollection : CollectionBase {
        ...
        static string GetDuplicateMessage(Type type, string elemName) {
            ...
            if (type == typeof(ServiceDescriptionFormatExtension)) 
                message = Res.GetString(Res.S("WebDuplicateFormatExtension"), elemName);
            else if (type == typeof(OperationMessage)) 
                message = Res.GetString(Res.S("WebDuplicateOperationMessage"), elemName);
            else if (type == typeof(Import)) 
                message = Res.GetString(Res.S("WebDuplicateImport"), elemName);
            else if (type == typeof(Message)) 
                message = Res.GetString(Res.S("WebDuplicateMessage"), elemName);
            else if (type == typeof(Port)) 
                message = Res.GetString(Res.S("WebDuplicatePort"), elemName);
            else if (type == typeof(PortType)) 
                message = Res.GetString(Res.S("WebDuplicatePortType"), elemName);
            else if (type == typeof(Binding)) 
                message = Res.GetString(Res.S("WebDuplicateBinding"), elemName);
            else if (type == typeof(Service)) 
                message = Res.GetString(Res.S("WebDuplicateService"), elemName);
            else if (type == typeof(MessagePart)) 
                message = Res.GetString(Res.S("WebDuplicateMessagePart"), elemName);
            else if (type == typeof(OperationBinding)) 
                message = Res.GetString(Res.S("WebDuplicateOperationBinding"), elemName);
            else if (type == typeof(FaultBinding)) 
                message = Res.GetString(Res.S("WebDuplicateFaultBinding"), elemName);
            else if (type == typeof(Operation)) 
                message = Res.GetString(Res.S("WebDuplicateOperation"), elemName);
            else if (type == typeof(OperationFault)) 
                message = Res.GetString(Res.S("WebDuplicateOperationFault"), elemName);
            else
                message = Res.GetString(Res.S("WebDuplicateUnknownElement"), type, elemName);
