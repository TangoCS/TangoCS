		...
		class PooledBufferManager : InternalBufferManager
		{
			...
			public override void ReturnBuffer(byte[] buffer)
			{
				...
				if (bufferPool != null)
				{
					...
					{
						throw Fx.Exception.Argument("buffer", Rs.S("BufferIsNotRightSizeForBufferManager"));
					}
		...
		class GCBufferManager : InternalBufferManager
		{
			...
			public override byte[] TakeBuffer(int bufferSize)
			{
				...
				catch (OutOfMemoryException exception)
				{
					...
					throw Fx.Exception.AsError(new InsufficientMemoryException("BufferAllocationFailed", exception));
				}
			}
