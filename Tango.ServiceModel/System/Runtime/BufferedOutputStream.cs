...
namespace System.Runtime
{
	...
	class BufferedOutputStream : Stream
	{
		...
		public override long Position {
			get {
				throw Fx.Exception.AsError(new NotSupportedException(Rs.S("SeekNotSupported")));
			}
			set {
				throw Fx.Exception.AsError(new NotSupportedException(Rs.S("SeekNotSupported")));
			}
		...
		{
			throw Fx.Exception.AsError(new NotSupportedException(Rs.S("ReadNotSupported")));
		}
		...
		void WriteCore(byte[] buffer, int offset, int size)
		{
			if (size < 0)
			{
				throw Fx.Exception.ArgumentOutOfRange("size", size, Rs.S("ValueMustBeNonNegative"));
			}
