...
namespace Nephrite.Metamodel.Model
{
    ...
    public class MM_ObjectPropertyMap : ClassMapping<MM_ObjectProperty>
    {
        ...
        public MM_ObjectPropertyMap()
        {
            ...
            Property(x => x.IsMultilingual, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
            Property(x => x.IsPrimaryKey, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
            Property(x => x.IsSystem, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
            Property(x => x.IsNavigable, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
            Property(x => x.IsAggregate, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
            Property(x => x.LowerBound, map => map.NotNullable(true));
            Property(x => x.UpperBound, map => map.NotNullable(true));
            Property(x => x.Expression, map =>
            {
                map.Column("\"EXPRESSION\"");

            });
            Property(x => x.IsReferenceToVersion, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
            Property(x => x.ValueFilter);
            Property(x => x.Precision, map =>
            {
            ...
            Property(x => x.LastModifiedDate, map => map.NotNullable(true));
            Property(x => x.LastModifiedUserID, map => map.NotNullable(true));
            Property(x => x.IsIdentity, map => { map.NotNullable(true); MappingConfig.BoolPropertyConfig(map); });
