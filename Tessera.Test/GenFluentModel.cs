	...
	public class ApplicationDomain
	{
		public static MetaSolution Init(MetaSolution s)
		{
			...
			s.AddClass<MM_ObjectType>("Класс")
				...
				.Reference<MM_Package>("Package", "Пакет", x => x.Required().InverseProperty("ObjectTypes"))
			...
			s.AddClass<MM_ObjectProperty>("Свойство")
				...
				.Reference<MM_ObjectType>("ObjectType", "Класс", x => x.Required().InverseProperty("Properties"))
			...
			s.AddClass<MM_FormField>("Поле формы")
				...
				.Reference<MM_ObjectProperty>("ObjectProperty", "Свойство", x => x.Required())
			...
			s.AddClass<MM_Codifier>("Кодификатор")
				...
				.Reference<MM_Package>("Package", "Пакет", x => x.Required().InverseProperty("Codifiers"))
				.OperationView() 
				.OperationEdit() 
				.OperationList() 
				.OperationDelete() 
			...
			s.AddClass<MM_CodifierValue>("Значение кодификатора")
				...
				.Attribute("SeqNo", "Порядковый номер", MetaIntType.NotNull())
				.Reference<MM_Codifier>("Codifier", "Кодификатор", x => x.Required().InverseProperty("Values"))
				.Operation("MoveUp", "Переместить вверх", x => x 
					.ParmInt("id")
					.ParmString("returnurl")
				)
			...
			s.AddClass<MM_FormFieldAttribute>("Атрибут поля формы")
				...
				.Reference<MM_FormField>("FormField", "Поле", x => x.Required().InverseProperty("Attributes"))
			;	
			...
			s.AddClass<MM_Method>("Метод")
				...
				.Reference<MM_ObjectType>("ObjectType", "Класс", x => x.Required().InverseProperty("Methods"))
			...
			s.AddClass<MM_MethodParameter>("Параметр метода")
				...
				.Reference<MM_Method>("Method", "Метод", x => x.Required().InverseProperty("MethodParameters"))
			;	
			s.AddClass<MM_FormFieldGroup>("Группа полей")
				...
				.Attribute("IsDeleted", "Удален", MetaBooleanType.NotNull())
				.Reference<MM_ObjectType>("ObjectType", "Класс", x => x.Required().InverseProperty("FormFieldGroups"))
			...
			s.AddClass<MM_Predicate>("Предикат")
				...
				.Reference<MM_ObjectType>("ObjectType", "Класс", x => x.Required().InverseProperty("DataValidations"))
				.OperationCreateNew(x => x 
					.ParmInt("parentid")
					.ParmString("returnurl")
				)
			...
			s.AddClass<SPM_Role>("Роль")
				...
				.Reference<SPM_C_RoleType>("RoleType", "Тип роли", x => x.Required())
			...
			s.AddClass<SPM_ActionAsso>("Ассоциатор ЗО")
				...
				.ReferenceKey<SPM_Action>("Action", "Action", x => x.Required())
				.ReferenceKey<SPM_Action>("ParentAction", "ParentAction", x => x.Required())
			;	
			s.AddClass<SPM_RoleAsso>("Наследование ролей")
				.ComputedAttribute("Title", "Title", MetaStringType.Null())
				.ReferenceKey<SPM_Role>("ParentRole", "ParentRole", x => x.Required())
				.ReferenceKey<SPM_Role>("Role", "Role", x => x.Required())
			;	
			s.AddClass<SPM_SubjectRole>("Роль пользователя")
				.ComputedAttribute("Title", "Title", MetaStringType.Null())
				.ReferenceKey<SPM_Role>("Role", "Role", x => x.Required())
				.ReferenceKey<SPM_Subject>("Subject", "Subject", x => x.Required())
			;	
			...
			s.AddClass<SPM_SubjectDelegate>("Делегирование полномочий")
				...
				.Reference<SPM_Subject>("DelegatedFrom", "От кого делегировано", x => x.Required())
				.Reference<SPM_Subject>("DelegatedTo", "Кому делегировано", x => x.Required())
				.OperationCreateNew() 
				.OperationEdit() 
				.OperationDelete() 
				.OperationList() 
			...
			s.AddClass<SPM_AvailableRoleForGrant>("Доступная роль для назначения пользователям")
				.ComputedAttribute("Title", "Title", MetaStringType.Null())
				.ReferenceKey<SPM_Role>("Role", "Role", x => x.Required())
				.ReferenceKey<SPM_Role>("RoleForGrant", "RoleForGrant", x => x.Required())
			;	
			...
			s.AddClass<TM_TaskParameter>("Параметр задачи")
				...
				.Attribute("SeqNo", "Порядковый номер", MetaIntType.NotNull())
				.Attribute("Title", "Название", MetaStringType.NotNull())
				.Reference<TM_Task>("Parent", "Задача", x => x.Required().InverseProperty("Parameters"))
				.OperationDelete() 
				.Operation("MoveUp", "Переместить вверх", x => x 
					.ParmInt("id")
					.ParmString("returnurl")
			...
			s.AddClass<TM_TaskExecution>("Запуск задачи")
				...
				.ComputedAttribute("Title", "Наименование", MetaStringType.Null())
				.Reference<TM_Task>("Task", "Задача", x => x.Required())
				.OperationList() 
				.OperationView() 
			;	
			...
			s.AddClass<N_Folder>("Папка")
				...
				.Reference<SPM_Subject>("Creator", "Создавший пользователь", x => x.Required())
			...
			s.AddClass<N_FileLibrary>("Библиотека")
				...
				.Reference<N_FileLibraryType>("FileLibraryType", "Тип", x => x.Required())
			;	
			s.AddClass<N_DownloadLog>("Журнал скачивания файлов")
				...
				.ComputedAttribute("Title", "Наименование", MetaStringType.Null())
				.Reference<N_File>("File", "Файл", x => x.Required())
				.OperationList() 
			;	
			...
			s.AddClass<N_ObjectPropertyChange>("Изменение свойства")
				...
				.Reference<N_ObjectChange>("ObjectChange", "Изменение объекта", x => x.Required().InverseProperty("PropertyChanges"))
				.OperationCreateNew() 
				.OperationEdit() 
				.OperationDelete() 
				.OperationList() 
			...
			s.AddClass<N_NavigItem>("Элемент меню")
				...
				.Reference<N_Navig>("Navig", "Меню", x => x.Required().InverseProperty("Items"))
			...
			s.AddClass<N_Node>("Страница сайта")
				...
				.Attribute("CreateDate", "Дата создания", MetaDateTimeType.NotNull())
				.Attribute("PublishDate", "Дата публикации", MetaDateTimeType.Null())
				.Reference<MM_FormView>("MasterPage", "Мастер-страница", x => x.Required())
				.OperationCreateNew() 
				.OperationEdit() 
				.OperationDelete() 
				.OperationView() 
			...
			s.AddClass<WF_Workflow>("Рабочий процесс")
				...
				.Reference<MM_ObjectType>("ObjectType", "Тип объекта", x => x.Required())
			...
			s.AddClass<WF_Activity>("Активность")
				...
				.Attribute("SeqNo", "Порядковый номер", MetaIntType.NotNull())
				.Reference<WF_Workflow>("Workflow", "Рабочий процесс", x => x.Required().InverseProperty("Activities"))
			...
			s.AddClass<WF_Transition>("Переход")
				...
				.Attribute("IsActive", "Действующий", MetaBooleanType.NotNull())
				.Attribute("SeqNo", "Порядковый номер", MetaIntType.NotNull())
				.Reference<WF_Activity>("Parent", "Исходная активность", x => x.Required())
				.Reference<WF_Activity>("TargetActivity", "Целевая активность", x => x.Required())
				.OperationCreateNew() 
				.OperationEdit() 
				.OperationDelete() 
				.OperationList() 
			...
			s.AddClass<C_FIAS_AddressObject>("Классификатор адресообразующих элементов")
				...
				.Reference<C_FIAS_ActualStatus>("ActStatus", "Статус актуальности адресного объекта ФИАС", x => x.Required())
				.Reference<C_FIAS_CenterStatus>("CentStatus", "Статус центра", x => x.Required())
				.Reference<C_FIAS_CurrentStatus>("CurrStatus", "Статус актуальности КЛАДР 4 (последние две цифры в коде)", x => x.Required())
				.Reference<C_FIAS_NormativeDocument>("NormDoc", "Внешний ключ на нормативный документ")
				.Reference<C_FIAS_OperationStatus>("OperStatus", "Статус действия над записью – причина появления записи", x => x.Required())
			;	
			...
			s.AddClass<C_FIAS_House>("Сведения по номерам домов улиц городов и населенных пунктов, номера земельных участков")
				...
				.Reference<C_FIAS_AddressObject>("Ao", "Родительский объект (улица, город, населеннй пункт и т.п.)", x => x.Required())
				.Reference<C_FIAS_EstateStatus>("EstStatus", "Признак владения", x => x.Required())
				.Reference<C_FIAS_NormativeDocument>("NormDoc", "Нормативный документ")
				.Reference<C_FIAS_HouseStateStatus>("StatStatus", "Состояние дома", x => x.Required())
				.Reference<C_FIAS_StructureStatus>("StrStatus", "Признак строения", x => x.Required())
			;	
			...
			s.AddClass<C_FIAS_HouseInterval>("Интервал дома")
				...
				.Attribute("UpdateDate", "Дата  внесения записи", MetaDateTimeType.NotNull())
				.ComputedAttribute("Title", "Title", MetaStringType.Null())
				.Reference<C_FIAS_AddressObject>("Ao", "Идентификатор объекта", x => x.Required())
				.Reference<C_FIAS_IntervalStatus>("IntStatus", "Статус интервала", x => x.Required())
				.Reference<C_FIAS_NormativeDocument>("NormDoc", "Нормативный документ")
			;	
			s.AddClass<C_FIAS_Landmark>("Описание мест расположения имущественных объектов")
				...
				.Attribute("UpdateDate", "Дата  внесения записи", MetaDateTimeType.NotNull())
				.ComputedAttribute("Title", "Title", MetaStringType.Null())
				.Reference<C_FIAS_AddressObject>("Ao", "Родительский объект (улици, город и т.п.)", x => x.Required())
				.Reference<C_FIAS_NormativeDocument>("NormDoc", "Нормативный документ")
			;	
