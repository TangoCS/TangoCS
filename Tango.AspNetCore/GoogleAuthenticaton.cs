		...
		public static void UseGoogleAuthentication(this IApplicationBuilder app, Action<GoogleAuthenticationOptions> configureOptions)
		{
			...
			app.Map("/googlelogin", b => {
				b.Run(async context => {
					...
					context.Response.Redirect($"https://accounts.google.com/o/oauth2/v2/auth?client_id={clientID}&response_type=code&scope=openid%20email%20profile&redirect_uri=https:%2F%2F{context.Request.Host.Value}{callback}&state={ret}");
