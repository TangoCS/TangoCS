	...
	public class ActionLink : SimpleLink
	{
		...
		public string Link(string title)
		{
			...
			if (Operation == null || ActionSPMContext.Current.Check(Operation.ID, 1))
				return AppWeb.Layout.Link(this);
			else
				return title;
		}
		public string Link()
		...
		public string ImageTextLink(string title)
		{
			Title = title;
			if (Operation == null || ActionSPMContext.Current.Check(Operation.ID, 1))
				return AppWeb.Layout.ImageLink(this) + "&nbsp;" + AppWeb.Layout.Link(this);
			else
				return AppWeb.Layout.Image(Image, title) + "&nbsp;" + title;
		}
		...
		{
			if (Operation == null || ActionSPMContext.Current.Check(Operation.ID, 1))
				return AppWeb.Layout.ImageLink(this);
			else
				return "";
		}
	}
